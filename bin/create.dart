...
Future main(List<String> args) async {
  ...
  parser.addOption('build-directory',
      abbr: "b", help: "Path to directory to store build artifacts (created by this script)", defaultsTo: "_build/");
  parser.addOption('output-file', abbr: "o", help: "Path to executable output of script", defaultsTo: 'runtime');
  parser.addOption('script',
      abbr: "s",
      help: "Path to the .dart script file to compile (contains a main function)",
      defaultsTo: "bin/main.dart");
  parser.addFlag('test', abbr: "t", help: "Include dev_dependencies when compiling, writes script as test");
  ...
  final ctx =
      BuildContext(library, buildDir, outputFile, File.fromUri(script).readAsStringSync(), forTests: results['test']);
