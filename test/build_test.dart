...
void main() {
  setUpAll(() async {
    ...
    final testPackagesUri = Directory.current.uri.resolve("test/").resolve("test_packages/");
    await Process.run(cmd, ["get", "--offline"],
        workingDirectory: testPackagesUri.resolve("application/").toFilePath(windows: Platform.isWindows),
        runInShell: true);
    await Process.run(cmd, ["get", "--offline"],
        workingDirectory: testPackagesUri.resolve("dependency/").toFilePath(windows: Platform.isWindows),
        runInShell: true);
  ...
  tearDownAll(() {
    Directory.fromUri(Directory.current.uri.resolve("tmp/")).deleteSync(recursive: true);
  });

  test("Non-compiled version returns mirror runtimes", () async {
    final output = await dart(Directory.current.uri.resolve("test/").resolve("test_packages/").resolve("application/"));
    expect(json.decode(output), {"Consumer": "mirrored", "ConsumerSubclass": "mirrored", "ConsumerScript": "mirrored"});
  });
...
Future<String> dart(Uri workingDir) async {
  final result = await Process.run("dart", ["bin/main.dart"],
      workingDirectory: workingDir.toFilePath(windows: Platform.isWindows), runInShell: true);
  return result.stdout.toString();
}

Future<String> runExecutable(Uri buildUri, Uri workingDir) async {
  final result = await Process.run(buildUri.toFilePath(windows: Platform.isWindows), [],
      workingDirectory: workingDir.toFilePath(windows: Platform.isWindows), runInShell: true);
  return result.stdout.toString();
}
